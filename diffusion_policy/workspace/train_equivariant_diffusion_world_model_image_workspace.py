...
class TrainEquivariantDiffusionWorldModelUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure training state
        self.global_step = 0
        self.epoch = cfg.training.start_epoch
        ...
        self.rot_video_recorder: VideoRecorder = VideoRecorder.create_h264(
            fps=cfg.task.env_runner.fps,
            codec='h264',
            input_pix_fmt='rgb24',
            crf=22,
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if cfg.training.freeze_encoder:
                    self.model.auto_encoder.eval()
                    self.model.auto_encoder.requires_grad_(False)
                ...
                if self.epoch>0 and (self.epoch % cfg.training.rollout_every) == 0:
                    ...
                    rotate_predict_video_path_list = []
                    with torch.inference_mode():
                        ...
                        for idx in selected_indices:
                            ...
                            predict_rotation_filename = pathlib.Path(self.output_dir).joinpath(
                            'media', 'rot90' + wv.util.generate_id() + ".mp4")
                            predict_rotation_filename.parent.mkdir(parents=False, exist_ok=True)
                            predict_rotation_filename = str(predict_rotation_filename) 
                            rotate_predict_video_path_list.append(predict_rotation_filename)
                            ...
                            predicted_image_history_rot = {
                                "image": torch.tensor(image_history, dtype=torch.float32).to(device).unsqueeze(0).rot90(k=self.rot90, dims=(-2, -1))
                            }
                            self.video_recoder.start(predict_filename)
                            self.rot_video_recorder.start(predict_rotation_filename)
                            for i in range(world_model.n_obs_steps):
                                image = dataset.replay_buffer['img'][start_idx + i].astype(np.uint8)
                                self.video_recoder.write_frame(image)
                                self.rot_video_recorder.write_frame(np.rot90(image, k=1))
                            for i in tqdm.trange(episode_length - world_model.n_obs_steps):
                                ...
                                predicted_results = world_model.predict_future(predicted_image_history, action) # B, T, C, H, W
                                ...
                                predicted_results_rot = world_model.predict_future(predicted_image_history_rot, action_rot) # B, T, C, H, W
                                ...
                                unnormalized_images_rot = torch.moveaxis(unnormalized_images_rot, 1, -1)
                                unnormalized_images_rot = (unnormalized_images_rot.detach().cpu().numpy() * 255).astype(np.uint8)
                                self.rot_video_recorder.write_frame(unnormalized_images_rot[0]) # only use the first frame
                            self.video_recoder.stop()
                            self.rot_video_recorder.stop()
                        
                            # image_history_ = image_history.copy()
                            # predicted_image_history = {
                            #     "image": torch.tensor(image_history_, dtype=torch.float32).to(device).unsqueeze(0)
                            # }
                            # predicted_results = world_model.predict_future(predicted_image_history, action.unsqueeze(0), last_latent=None)
                            
                            # predicted_image_history_rot = {
                            #     "image": torch.tensor(image_history, dtype=torch.float32).to(device).unsqueeze(0).rot90(k=1, dims=(3, 4))
                            # }
                            # predicted_results_r = world_model.predict_future(predicted_image_history_rot, action.unsqueeze(0) @ rotation_matrix.T, last_latent=None)
                            # print(abs(predicted_results['predicted_future'].rot90(k=2,dims=(3,4)) - predicted_results_r['predicted_future'])[0,0])
                            # sad
                            
                            
                            # print((pred_rotate - pred.rot90(k=1, dims=(2,3))).max())
                            # asd
                    ...
                    for i, video_path in enumerate(rotate_predict_video_path_list):
                        sim_video = wandb.Video(video_path)
                        rollout_log[f'predict_video_rotation_{i}'] = sim_video
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        future_action = (future_action-256.0)/256.0
                        future_action = torch.concat([future_action[:, :, 1:], future_action[:, :, :1]], dim=2)
                        ...
                        # rot90
                        history_obs_dict_rot = {
                            'image': obs_dict['image'][:,:self.model.n_obs_steps,...].rot90(k=1, dims=(3, 4))
                        }
                        future_action_rot = future_action @ self.rotation_matrix.T.to(device)
                        result_rot = world_model.predict_future(history_obs_dict_rot, future_action_rot)
                        pred_future_images_rot = result_rot["predicted_future"]
                        
                        # log predicted and ground truth future images
                        for i in range(3):
                            idx = np.random.randint(0, pred_future_images.shape[0]-1)
                            pred_images = (pred_future_images[idx].cpu().numpy() * 255).astype(np.uint8)
                            pred_images = np.moveaxis(pred_images, 1, -1)
                            pred_images_rot = (pred_future_images_rot[idx].cpu().numpy() * 255).astype(np.uint8)
                            pred_images_rot = np.moveaxis(pred_images_rot, 1, -1)
                            gt_images = (target_future_images[idx].cpu().numpy() * 255).astype(np.uint8)
                            ...
                            pred_wandb_img_rot = wandb.Image(pred_images_rot[0], caption="Predicted future image rotate")
                            step_log[f'train_pred_future_image_{i}'] = pred_wandb_img
                            step_log[f'train_gt_future_image_{i}'] = gt_wandb_img
                            step_log[f'train_pred_future_image_rotated_{i}'] = pred_wandb_img_rot
                        del batch
                        del obs_dict
                        del gt_action
                        del result
