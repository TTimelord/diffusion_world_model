...
class EvalDiffusionWorldModelLatentUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.test_rot = cfg.test_rotate
        if self.test_rot:
            self.rot90 = cfg.rot90
        ...
        with torch.inference_mode():
            ...
            for index, idx in enumerate(selected_indices):
                ...
                if self.test_rot:
                    ...
                    predicted_image_trajectory_rot[:world_model.n_obs_steps] = gt_image_trajectory[:world_model.n_obs_steps].rot90(k=self.rot90, dims=(2, 3))
                ...
                if self.test_rot:
                    predicted_image_history_rot = {
                        "image": gt_image_trajectory[:world_model.n_obs_steps].unsqueeze(0).rot90(k=self.rot90, dims=(3, 4))
                    }
                ...
                for i in tqdm.trange(episode_length - world_model.n_obs_steps - depth):
                    ...
                    for k in range(depth):
                        predicted_results = world_model.predict_future(predicted_image_history, action[:, k:k+1], last_latent) # B, T, C, H, W
